...
//MP3-Player - DFRobotDFPlayer 1.0.5:
...
int16_t SOUNDBASENR[16] = {AUDIO_BASENR0,AUDIO_BASENR1,AUDIO_BASENR2,AUDIO_BASENR3,AUDIO_BASENR4,AUDIO_BASENR5, 
                           AUDIO_BASENR6,AUDIO_BASENR7,AUDIO_BASENR8,AUDIO_BASENR9,AUDIO_BASENR10,AUDIO_BASENR11,
                           AUDIO_BASENR12,AUDIO_BASENR_VICKI, AUDIO_BASENR_HANS, AUDIO_BASENR_RANDOM};
int AUDIO_HOUR = -1;
...
bool ONOFF_BUTTON_BOOL = false;
uint16_t ZEIGE_KURZ = 0;
int ModeSequenz = 0;
int modeExtraTime = 0;
int Modecount = 0;
...
unsigned long wetteransagets = 0;
...
uint16_t minLdrValue = 200; // The ESP will crash if minLdrValue and maxLdrValue are equal due to an error in map()
uint16_t maxLdrValue = 2500;
#endif
...
#ifdef PIN_MODE_BUTTON_TOUCH
uint8_t mode_touchthreshold = 0;
uint8_t mode_value = 0;
bool touchedMode = false;
bool enableMode = false;
volatile unsigned long sinceLastTouchMode = 0;
#endif

#ifdef PIN_ONOFF_BUTTON_TOUCH
uint8_t onoff_touchthreshold = 0;
uint8_t onoff_value = 0;
bool touchedOnOff = false;
bool enableOnOff = false;
volatile unsigned long sinceLastTouchOnOff = 0;
#endif
...
void setup()
{
 ...
 settings.mySettings.corner_color = BLUE;
...
#ifdef PIN_MODE_BUTTON_TOUCH
   mode_value = touchRead(PIN_MODE_BUTTON_TOUCH);
   mode_touchthreshold = mode_value - MODE_TOUCH_SENS - 2;
   Serial.print(F("Setting up Mode-Touch Button. Value/Threshold: "));
   Serial.print(mode_value);
   Serial.print(F(" / "));
   Serial.println(mode_touchthreshold);
   touchAttachInterrupt(PIN_MODE_BUTTON_TOUCH, touchModewasActivated, mode_touchthreshold);
#endif

#ifdef PIN_ONOFF_BUTTON
	Serial.println(F("Setting up ON/OFF-Button."));
	pinMode(PIN_ONOFF_BUTTON, INPUT_PULLUP);
	attachInterrupt(digitalPinToInterrupt(PIN_ONOFF_BUTTON), buttonOnOffInterrupt, FALLING);
#endif

#ifdef PIN_ONOFF_BUTTON_TOUCH
   onoff_value = touchRead(PIN_ONOFF_BUTTON_TOUCH);
   onoff_touchthreshold = onoff_value - ONOFF_TOUCH_SENS - 2;
   Serial.print(F("Setting up OnOff-Touch Button. Value/Threshold: "));
   Serial.print(onoff_value);
   Serial.print(F(" / "));
   Serial.println(onoff_touchthreshold);
   touchAttachInterrupt(PIN_ONOFF_BUTTON_TOUCH, touchOnOffwasActivated, onoff_touchthreshold);
#endif
  ...
  HWSerial_1.begin(9600,SERIAL_8N1,PIN_AUDIO_TX, PIN_AUDIO_RX);
  delay(500);
  Mp3Player.begin(HWSerial_1); // Starten der seriellen Kommunikation zwischen dem D1 Mini und dem MP3 Modul.
...
int  wifiversuche = 0;
 //  WiFi.disconnect(true);
  WiFi.setHostname(settings.mySettings.systemname.c_str());
  WiFi.begin(settings.mySettings.wlanssid.c_str(), settings.mySettings.wlanpw.c_str());
//  WiFi.mode(WIFI_STA);
  delay (500);
    Serial.print("Verbinde mit WLAN: ");
    Serial.println(settings.mySettings.wlanssid);
  while (WiFi.status() != WL_CONNECTED  && wifiversuche < WIFI_SETUP_TIMEOUT * 2 ) {
    delay(500);
    Serial.print(".");
    wifiversuche++;
    if ( (wifiversuche+1 )%30 == 0 ) 
    {
      Serial.println(" ");
      WiFi.disconnect(true);
      delay (100);
...
//    Serial.println(STA_pass);    
    ...
    if (!MDNS.begin(settings.mySettings.systemname.c_str())) {
...
#endif
    ArduinoOTA.setHostname(settings.mySettings.systemname.c_str());
    ArduinoOTA.begin();
  }

// Highscores aus NVS holen
  ...
  delay (500);
	...
	if (WiFi.isConnected())
	{
		// Set ESP time from NTP
		time_t tempNtpTime = ntp.getTime(settings.mySettings.ntphost);
		if (tempNtpTime)
		{
			errorCounterNTP = 0;
...
#endif
    delay(1000); 
    retcodeOutdoorWeather = outdoorWeather.getOutdoorConditions(settings.mySettings.latitude,settings.mySettings.longitude);
    if ( retcodeOutdoorWeather > 0 ) 
    {
#ifdef DEBUG_OW
     Serial.println(F("yrno Error"));
#endif
...
// Sekunden Task
#ifdef SEKUNDENLEDS
xTaskCreatePinnedToCore(SekundenTask,"CPU_1",10000,NULL,1,&SekundenHnd,1);
#endif
...
// SunRise
  ...
  sunSetTime = timeZone.toLocal(sr.setTime);
    ...
    WLAN_reconnect = 0;
    maxesptimedrift = 0;
    minFreeBlockSize = ESP.getMaxAllocHeap();
    codeline = __LINE__;
#ifdef DEBUG
	...
	{
// Check ob wir nach einem Neustart ab 10 Minuten die aktuelle Zeit haben. Wenn nicht dann Neustart.

    if ( millis()/1000 > 600 && millis()/1000 < 900 && year() < 2010 ) 
    {
      Serial.println(F("Nach 10 Minuten konnte keine aktuelle Zeit geholt werden! "));
  ...
  //    Serial.println ( animation);
    ...
    // ************************************************************************
    if ( aktMinute % 10 == 4 && timeZone.toUTC(now()) > LastOutdoorWeatherTime + YRNO_PERIODE && errorCounterOutdoorWeather < YRNO_MAX_ERROR )  
    {
      if (WiFi.isConnected())
      {
#ifdef DEBUG_OW
...
// Wenn Off und der Mode-Button/Touch wird gedrückt Anzeige für x Sekunden an
    if ( ZEIGE_KURZ > 1 )
    {
       ZEIGE_KURZ--;
    }
    else if ( ZEIGE_KURZ == 1 )
    {
      setMode(MODE_BLANK);
      ZEIGE_KURZ--;
    }
...
#ifdef PIN_MODE_BUTTON_TOUCH
  if ( enableMode && aktSecond%7 == 0 ) newMode_threshold();  // alle 7 Sekunde wird ein neuer Mode Threshold ermittelt
#endif

#ifdef PIN_ONOFF_BUTTON_TOUCH
  if ( enableOnOff && aktSecond%7 == 0 ) newOnOff_threshold(); // alle 7 Sekunde wird ein neuer OnOff Threshold ermittelt
#endif
...
// *********************************************************************************		
	...
	// ************************************************************************
#ifdef SEKUNDENLEDS
  if ( mode == MODE_TIME ) vTaskResume(SekundenHnd); 
  else 
  {
    vTaskSuspend(SekundenHnd);
    sekledDriver.clear();
    sekledDriver.show(); 
  }
#endif
  ...
  {
    touchedMode = false;
    if (touchDelayComp(sinceLastTouchMode ) && enableMode )
    {
      enableMode  = false;
      ...
      sinceLastTouchMode = millis();
    }
  }

  if ( !enableMode ) 
  {
    if ( touchRead(PIN_MODE_BUTTON_TOUCH) > mode_touchthreshold  ) 
    {
      enableMode = true;
#ifdef DEBUG_TOUCH
      Serial.println("Touch enableMode");
#endif
    } 
    else
    {
      if ( aktSecond%5 == 0 ) mode_touchthreshold=mode_touchthreshold-1; 
    }   
  }
#endif // Ende PIN_MODE_BUTTON_TOUCH
  ...
  {
    touchedOnOff = false;
    if (touchDelayComp(sinceLastTouchOnOff ) && enableOnOff )
    {
      enableOnOff  = false;
      
#ifdef DEBUG_TOUCH
      Serial.println("Touch OnOff");
#endif      
      buttonOnOffPressed();
      sinceLastTouchOnOff = millis();
    }
  }

  if ( !enableOnOff ) 
  {
    if ( touchRead(PIN_ONOFF_BUTTON_TOUCH) > onoff_touchthreshold  ) 
    {
      enableOnOff = true;
#ifdef DEBUG_TOUCH
      Serial.println("Touch enableOnOff");
#endif
    } 
    else
    {
      if ( aktSecond%5 == 0 ) onoff_touchthreshold=onoff_touchthreshold-1; 
    }
  }
#endif // Ende PIN_ONOFF_BUTTON_TOUCH

#ifdef PIN_ONOFF_BUTTON
  if ( ONOFF_BUTTON_BOOL) 
  {
    ONOFF_BUTTON_BOOL = false;
    buttonOnOffPressed();
  }
#endif
...
// ############# Wetteransage Start
#ifdef AUDIO_SOUND
#define WETTERBUSYPINDELAY 700

    if ( ModeSequenz == 2 && !autoModeChangeFlag ) {
      AUDIO_FILENR = ANSAGEBASE + 400;              // Wetter
      Play_MP3(AUDIO_FILENR,false,0);
      wetteransagets = millis();
      WetterAnsageSchritt = 2; //Start Ansage Wetter
    }
    
    if ( WetterAnsageSchritt == 2 && ( millis() - wetteransagets) > WETTERBUSYPINDELAY  && digitalRead(PIN_AUDIO_BUSY) && !autoModeChangeFlag ) {
      AUDIO_FILENR = ANSAGEBASE + 410 + outdoorWeather.symbol_id; 
      Play_MP3(AUDIO_FILENR,false,0);             // Wetteransage
      wetteransagets = millis();
      WetterAnsageSchritt = 3;  //Ende Ansage Wetter Start Ansage Wetterlage
    }

    if ( WetterAnsageSchritt == 3 && ( millis() - wetteransagets) > WETTERBUSYPINDELAY  && digitalRead(PIN_AUDIO_BUSY) && !autoModeChangeFlag  )
    {
      if ( outdoorWeather.windspeed <= 0.5 )                                     AUDIO_FILENR = ANSAGEBASE + 490;      //windstill
      if ( outdoorWeather.windspeed > 0.5 && outdoorWeather.windspeed <= 4.0 )   AUDIO_FILENR = ANSAGEBASE + 491;      //leichte Brise
      if ( outdoorWeather.windspeed > 4.0 && outdoorWeather.windspeed <= 8.0 )   AUDIO_FILENR = ANSAGEBASE + 492;      //mäßiger-schwacher Wind
      if ( outdoorWeather.windspeed > 8.0 && outdoorWeather.windspeed <= 14.0 )  AUDIO_FILENR = ANSAGEBASE + 493;      //starker Wind
      if ( outdoorWeather.windspeed > 14.0 && outdoorWeather.windspeed <= 18.0 ) AUDIO_FILENR = ANSAGEBASE + 494;      //stürmischer Wind
      if ( outdoorWeather.windspeed > 18 )                                       AUDIO_FILENR = ANSAGEBASE + 495;      //Sturm
      Play_MP3(AUDIO_FILENR,false,0);             // Wind
      wetteransagets = millis();
      WetterAnsageSchritt = 4; // Ende Ansage Wetterlage Start Ansage Wind
    }

    if ( WetterAnsageSchritt == 4 && ( millis() - wetteransagets) > WETTERBUSYPINDELAY  && digitalRead(PIN_AUDIO_BUSY) && !autoModeChangeFlag ) 
    {
...
// ############# Wetteransage Ende
    ...
    {
      animation = F("WETTER_");
      animation += getWeatherAnimation(outdoorWeather.animation_code);
...
#ifdef DEBUG
    Serial.printf("WetterAnsageSchritt: %i\n", WetterAnsageSchritt);
#endif

    if ( showAnimation(brightness) && WetterAnsageSchritt < 5  )
    {
      modeExtraTime = 500;
      modeTimeout = millis();
    }
...
//					matrix[2 + z] |= (lettersBig[feedText[feedPosition + 2] - 32][z] << y - 1) & 0b1111111111100000;
    ...
    break;
      ...
      else if ( playanimation )
      {
        ...
        playanimation = false;
        if ( settings.mySettings.colorChange > COLORCHANGE_NO ) settings.mySettings.color = aktHour;
...
void moveSeriell(uint16_t screenBufferOld[], uint16_t screenBufferNew[], uint8_t color, uint8_t brightness, uint8_t sertype)
{
  ...
  for (int a = 0; a <= 109; a++)
  {
    ...
    for (int b = 0; b <= 109; b++)
    {
      ...
      pointerbuffer = seriellpointer[sertype][b];
      ...
      pointerbuffer = seriellpointer[sertype][s_old];
      ...
      pointerbuffer = seriellpointer[sertype][s_new];
...
/*
  uint8_t bogen[10][12] = {
             ...
             {0      ,0      ,RED    ,RED    ,ORANGE ,ORANGE ,ORANGE ,RED    ,RED    ,0      ,0     },
             {0      ,RED    ,ORANGE ,ORANGE ,YELLOW ,YELLOW ,YELLOW ,ORANGE ,ORANGE ,RED    ,0     },
             {RED    ,ORANGE ,YELLOW ,YELLOW ,GREEN  ,GREEN  ,GREEN  ,YELLOW ,YELLOW ,ORANGE ,RED   },
             {ORANGE ,YELLOW ,GREEN  ,GREEN  ,BLUE   ,BLUE   ,BLUE   ,GREEN  ,GREEN  ,YELLOW ,ORANGE},
             {YELLOW ,GREEN  ,BLUE   ,BLUE   ,MAGENTA,MAGENTA,MAGENTA,BLUE   ,BLUE   ,GREEN  ,YELLOW},
             {GREEN  ,BLUE   ,MAGENTA,MAGENTA,99     ,99     ,99     ,MAGENTA,MAGENTA,BLUE   ,GREEN },
             {BLUE   ,MAGENTA,99     ,99     ,99     ,99     ,99     ,99     ,99     ,MAGENTA,BLUE  },
...
//        ledDriver.setPixel(x, y, color, brightnessBuffer[y][x]);
    ...
    {
      delay(int((500 - brightness) / brightness ));
    }
		else
		{
		  delay(int((12 * TRANSITION_SPEED + 255 - brightness) / brightness));
		}
		ledDriver.show();
	}
...
#endif
#ifdef AUDIO_SOUND
  if (  ZEIGE_KURZ > 0 ) Play_MP3(ANSAGEBASE+41,false,0); // Ein
  else Play_MP3(700,true,0);  // OK Sound blub
#endif
	// Switch off alarm
#ifdef BUZZER
	if (alarmOn)
...
#ifdef PIN_MODE_BUTTON_TOUCH
ICACHE_RAM_ATTR void touchModewasActivated() { touchedMode = true; }

void newMode_threshold()
{
  mode_value = touchRead(PIN_MODE_BUTTON_TOUCH);
  uint8_t threshold;
  bool update_threshold = false;
  threshold = mode_value - MODE_TOUCH_SENS - 1;
  if ( threshold > mode_touchthreshold && enableMode  ) { mode_touchthreshold++; update_threshold=true;}
  if ( threshold < mode_touchthreshold && enableMode  ) { mode_touchthreshold--; update_threshold=true;}
  ...
  {
    #ifdef DEBUG_TOUCH
    Serial.print(F("Mode-Touch Button. Neuer Threshold: "));
    Serial.print(mode_touchthreshold);
    Serial.print(F(" Aktueller Wert: "));
...
ICACHE_RAM_ATTR void touchOnOffwasActivated() { touchedOnOff = true; }

void newOnOff_threshold()
{
  onoff_value = touchRead(PIN_ONOFF_BUTTON_TOUCH);
  uint8_t threshold;
  bool update_threshold = false;
  threshold = onoff_value - ONOFF_TOUCH_SENS - 1;
  if ( threshold > onoff_touchthreshold && enableOnOff  ) { onoff_touchthreshold++; update_threshold=true;}
  if ( threshold < onoff_touchthreshold && enableOnOff  ) { onoff_touchthreshold--; update_threshold=true;}
  ...
  {
    #ifdef DEBUG_TOUCH
    Serial.print(F("OnOff-Touch Button. Neuer Threshold: "));
    Serial.print(onoff_touchthreshold);
    Serial.print(F(" Aktueller Wert: "));
    Serial.println(onoff_value);
    #endif
    touchAttachInterrupt(PIN_ONOFF_BUTTON_TOUCH, touchOnOffwasActivated, onoff_touchthreshold );
  }
}

#endif     //PIN_ONOFF_BUTTON_TOUCH
...
#ifdef YRNO
String getWeatherAnimation(String animation_code)
{
  String tagnacht;
  String wnummer;
  uint8_t winummer;
  ...
  wnummer=animation_code;
  
  if ( wnummer == "03" && outdoorWeather.clouds > 80 ) wnummer = "04";
 
  winummer = wnummer.toInt();
  if ( now() > sunRiseTime && now() < sunSetTime ) tagnacht = "d";
  else tagnacht = "n";
...
{
  if ( ESP.getMaxAllocHeap() < minFreeBlockSize ) 
  {
    minFreeBlockSize = ESP.getMaxAllocHeap();
    codeline = codezeile;
#ifdef DEBUG
...
//	webServer.onNotFound(handleNotFound);
	...
	webServer.on("/", []() { if (WiFi.getMode() == WIFI_STA) handleRoot(); else handleButtonSettings(); });
  ...
  webServer.on("/commitSettings", HTTP_POST, handleCommitSettings);
...
bool captivePortal(String path)
{
  if (path.startsWith("/generate_204") || path.startsWith("/fwlink") ) 
  {
    if (webServer.hostHeader() != settings.mySettings.systemname ) {
#ifdef DEBUG
      Serial.print ("captive portal: ");
      Serial.println ( webServer.hostHeader());
#endif
      webServer.sendHeader("Location", String("http://" + settings.mySettings.systemname + "/handleButtonSettings" ), true);
      webServer.send(302, "text/plain", "");
      webServer.client().stop();
    }
    else
    {
      if (WiFi.getMode() == WIFI_STA) handleRoot(); else handleButtonSettings();
    }
    return true;
  }
  return false;
...
void handleRoot()
{
  ...
  webServer.setContentLength(CONTENT_LENGTH_UNKNOWN);
	...
	message += F("<html><head>");
	message +=	"<title>" + settings.mySettings.systemname + "</title>";
    ...
    "<script src=\"jquery-3.1.1.min.js\"></script>\n"
		"</head>"
		"<body>");
	message += "<h1>" + settings.mySettings.systemname + "</h1>\n";
...
// Abschnitt Mondphase
  ...
  message += F("<img id=\"Sonnen\" title=\"Sonnenaufgang\" src=\"/web_sonnenaufgang.jpg\" alt=\"Sonnenaufgang\">");
  message += F("</th>\n");
  
  message += F("<th style=\"width:30%\">");
  message += F("<img id=\"Sonnen\" title=\"Sonnenuntergang\" src=\"/web_sonnenuntergang.jpg\" alt=\"Sonnenuntergang\">");
  message += F("</th>\n");
  ...
  if (minute(sunRiseTime) < 10) message += F("0");
  ...
  if (minute(sunSetTime ) < 10) message += F("0");
...
message += F("<span style=\"font-size:24px;\">");
  if ( luftdrucktendenz_web == 0 ) message += F(" -");
...
// Abschnitt Außentemperatur + Luftfeuchtigkeit
  ...
  message += F(" &#13223;<br></span><span style=\"font-size:20px;\">Regenmenge: ");
  message += String(outdoorWeather.regenmenge);
  message += F(" ");
  message += outdoorWeather.regenmenge_units;
  message += F("</span><br><span style=\"font-size:20px;\">");
  message += outdoorWeather.symbol_text;
  message += F("</span><br>");

  message += F("<img id=\"Wettersymbole\" title=\"Wetter\" src=\"https://api.met.no/images/weathericons/svg/");
  message += outdoorWeather.symbol_code;
  message += F(".svg\" alt=\"Wetter\">");
...
void debugClock()
{
  ...
  String message = F("<!doctype html>"
    "<html>"
    "<head>");
    message += "<title>" + settings.mySettings.systemname + " Info</title>";  
    message += F("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">"
  ...
  message +="<br>tgl. Stunde: " + String(randomHour) + ", stdl. Minute: " + String(randomMinute) +
   ...
   "<br>MaxFreeBlockSize: " + String(ESP.getMaxAllocHeap()) + " bytes"
...
//   "<br>HeapFragmentation: " + String(ESP.getHeapFragmentation()) + " %"
...
#endif
  message += "<br>NTP-Server: " + settings.mySettings.ntphost +
...
#ifdef YRNO
  message += "<br>Error (yrno): " + String(errorCounterOutdoorWeather) + " <small>(Code: " + String(retcodeOutdoorWeather,BIN) + ")</small>";

  message += "<br>Letzter yrno Update: " + String(hour(timeZone.toLocal(LastOutdoorWeatherTime))) + ":";
  ...
  message += "<br>WetterInfo 1: <small>ID: " + String(outdoorWeather.symbol_id) + " ICON: " + outdoorWeather.symbol_code + " CLOUDS: " + outdoorWeather.clouds + "</small>";
#endif
#ifdef SENSOR_BME280
...
#ifdef PIN_MODE_BUTTON_TOUCH
  message += "<br>Mode Touch Value/Threshold: " + String(mode_value) + "/" + String(mode_touchthreshold);
#endif
#ifdef PIN_ONOFF_BUTTON_TOUCH
...
#endif
#if defined(PIN_ONOFF_BUTTON) || defined(PIN_MODE_BUTTON)
  message += F("BUTTONS ");
#else
  message += F("<s>BUTTONS</s> ");
#endif
#ifdef PIN_ONOFF_BUTTON_TOUCH
  message += F("ONOFF-TOUCH ");
#else
  message += F("<s>ONOFF-TOUCH</s> ");
#endif
#ifdef PIN_MODE_BUTTON_TOUCH
  message += F("MODE-TOUCH ");
#else
  message += F("<s>MODE-TOUCH</s> ");
#endif
  message += F("</body></html>");
  webServer.send(200, "text/html", message);
}
...
void handleButtonSettings()
{
  ...
  Serial.println("Settings pressed.");
    ...
    if (WiFi.getMode() != WIFI_STA) stamode = false;
  ...
  String message = F("<!doctype html>"
    "<html>"
    "<head>");
    message += F("<title>"); message += settings.mySettings.systemname; message += F(" Einstellung</title>");
    message += F("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n"
    "<meta charset=\"UTF-8\">\n");
    
    if ( stamode )
    {
      message += F("<link rel=\"icon\" type=\"image/png\" sizes=\"192x192\"  href=\"/android-icon-192x192.png\">\n"
      "<link rel=\"icon\" type=\"image/png\" sizes=\"32x32\" href=\"/favicon-32x32.png\">\n"
      "<link rel=\"icon\" type=\"image/png\" sizes=\"96x96\" href=\"/favicon-96x96.png\">\n"
      "<script src=\"jquery-3.1.1.min.js\"></script>\n");
    }
    ...
    "</head>"
    "<body>\n");
    message += F("<h2>"); message += settings.mySettings.systemname; message += F(" Settings</h2>");
// ################################################# sende html Teil 1
    webServer.send(200, "text/html", message);
    message = "";
    delay(0);
// ###################################################################
    message += F("<form action=\"/commitSettings\" method=\"POST\">\n"
    "<table>\n");
  // ------------------------------------------------------------------------
  if (stamode)
  {
...
// ------------------------------------------------------------------------
  message += F("<tr><td>"
    "Sekunden Farbenwechsel:"
    "</td><td>"
    "<input type=\"radio\" id=\"scc0\" name=\"scc\" value=\"0\"");
  if (settings.mySettings.sekmode == 0) message += F(" checked");
  message += F("> aus "
    "<br>"
    "<input type=\"radio\" id=\"scc1\" name=\"scc\" value=\"1\"");
  if (settings.mySettings.sekmode == 1) message += F(" checked");
  message += F("> Farbverlauf/h "
    "<br>"
    "<input type=\"radio\" id=\"scc2\" name=\"scc\" value=\"2\"");
  if (settings.mySettings.sekmode == 2) message += F(" checked");
  message += F("> wie Hauptfarbe "
    "<br>"
    "<input type=\"radio\" id=\"scc3\" name=\"scc\" value=\"3\"");
  if (settings.mySettings.sekmode == 3) message += F(" checked");
  message += F("> Farbe "
    "<input type=\"color\" id=\"scs\" name=\"scs\" value=\"");
  message += num_to_string(settings.mySettings.sekcolor);
  message += F("\">"
    "</td></tr>\n");
  // ------------------------------------------------------------------------
...
//#########################################################
  ...
  message += F("<tr><td>"
    "Zeitserver:"
    "</td><td>");
  message += F("<input type=\"search\" list=\"ntpserver\" placeholder=\""); message += settings.mySettings.ntphost + "\" name=\"ntphost\">";
  ...
  message += F("<tr><td>"
    "Systemname:<br>SSID:<br>WiFiPasswort:<br><br>"
    "</td><td>");
  message += F("<input type=\"text\" value=\""); 
  message += settings.mySettings.systemname; 
  message += F("\" name=\"sysname\" id=\"sysname\" minlength=\"3\" maxlength=\"29\">");
//  message += F("<br><input type=\"text\" value=\""); 
//  message += settings.mySettings.wlanssid; 

  message += "<br><input type=\"search\" id=\"wlanssid\" list=\"wlanliste\" placeholder=\"" + ownssid +"\" name=\"wlanssid\" minlength=\"3\" maxlength=\"29\"";
  if (WiFi.getMode() != WIFI_STA) message += F(" autofocus");
  ...
  message += settings.mySettings.wlanpw; 
  message += F("\" id=\"wlanpw\" id=\"wlanpw\" name=\"wlanpw\" minlength=\"6\" maxlength=\"29\">");
...
#ifdef YRNO
  message += F("<tr><td>"
    "Standort<br>Latitude:<br>Longitude:<br>H&oumlhe:"
    "</td><td><br>");
  message += F("<input type=\"number\" value=\""); 
  message += String(settings.mySettings.latitude); 
  message += F("\" name=\"latitude\" min=\"-90\" max=\"90\" step=any >"
             "<br><input type=\"number\" value=\""); 
  message += String(settings.mySettings.longitude); 
  message += F("\" name=\"longitude\" min=\"-180\" max=\"180\" step=any >"
            "<br><input type=\"number\" value=\""); 
  ...
  message += F("</table>\n"
    ...
    "<button title=\"Save Settings.\" id=\"save_btn\" ><span style=\"color:White;font-size:14px;\">&#128077; speichern</span></button>"
    "</form>\n");
...
//    "$(\"#shwlanpw\").click(function(){"
//      "if ( $(\"#wlanpw\").attr(\"type\") == \"password\") { "
//      "$(\"#wlanpw\").attr(\"type\",\"text\")"
//      "}else{"
//      "$(\"#wlanpw\").attr(\"type\",\"password\")"
//      "}"
//    "});\n"    

    "save_btn.addEventListener('click', () => {"
//    "console.log($(\"#sysname\").prop('value'));"
//    "console.log($(\"#wlanssid\").prop('value'));"
//    "console.log($(\"#wlanpw\").prop('value'));"
      "if ( $(\"#sysname\").prop('value') != \"");
  message += settings.mySettings.systemname;
  message += F("\" || ( "
           "$(\"#wlanssid\").prop('value') != \"");
  message += settings.mySettings.wlanssid;
  message += F("\" && $(\"#wlanssid\").prop('value') != \"\" ) || "
           "$(\"#wlanpw\").prop('value') != \"");
  message += settings.mySettings.wlanpw;
  message += F("\" )"
      "{"
      "if (!confirm(`Wirklich die WLAN Verbindungsdaten ändern?\\nDanach erfolgt ein Neustart!`)) event.preventDefault();"
      "}"
    "});"
...
// ------------------------------------------------------------------------
  	...
  	else
    {
      ...
      if ( ldrValue <= 2000 ) lastLdrValue = 4000;
...
// ------------------------------------------------------------------------
 ...
 // ------------------------------------------------------------------------
   #ifdef SEKUNDENLEDS
     String webargvalue = webServer.arg("scs");
     webargvalue.toUpperCase();
     settings.mySettings.sekcolor = string_to_num(webargvalue);
     settings.mySettings.sekmode=webServer.arg("scc").toInt();
   #endif
 // ------------------------------------------------------------------------
 ...
 // ------------------------------------------------------------------------
    ...
    {
      settings.mySettings.ntphost = webServer.arg("ntphost");
      aktntptime = true;
    }
...
// WLAN Parameter
  ...
  {
    settings.mySettings.systemname=webServer.arg("sysname");
    neustart = true;
  }
  if ( webServer.arg("wlanssid").length() > 3 && webServer.arg("wlanssid") != settings.mySettings.wlanssid )
  {
    settings.mySettings.wlanssid=webServer.arg("wlanssid");
    neustart = true;
  }
  if ( webServer.arg("wlanpw").length() > 6 && webServer.arg("wlanpw") != settings.mySettings.wlanpw )
  {
    settings.mySettings.wlanpw=webServer.arg("wlanpw");
    neustart = true;
  }
// ------------------------------------------------------------------------   
	settings.saveToNVS();
...
//################ Sekunden LED Task ###############################
#ifdef SEKUNDENLEDS
void SekundenTask( void * parameter ) 
{ 
  uint8_t _aktSecond = second();
  uint8_t _prevSecond = second();
  uint8_t aktled;
  uint8_t aktledbck = aktled;
  TickType_t xLastWakeTime;
  xLastWakeTime = xTaskGetTickCount ();
  for (;;) 
  { 
    _prevSecond = second();
    if ( _aktSecond != _prevSecond && mode == MODE_TIME && settings.mySettings.sekmode != 0)
    {
      _aktSecond = _prevSecond;
      aktled = _aktSecond / ( 60 / NUMPIXELS_SEK );
//      Serial.printf("1.aktled: %i\n", aktled);
      if ( aktled + SEK0POSLED + 1  >= NUMPIXELS_SEK ) 
      {
        
        aktled = -1 * (NUMPIXELS_SEK - aktled - SEK0POSLED - 1);
      }
      else
      {
        aktled = aktled + SEK0POSLED + 1;
      }
//      Serial.printf("2.aktled: %i\n", aktled);
      
      setsekledPixel(aktled,aktledbck);
...
//    Serial.printf("xLastWakeTime: %i\n", xLastWakeTime);
    if ( mode != MODE_TIME || settings.mySettings.sekmode == 0)
    {
      sekledDriver.clear();
      sekledDriver.show();  
...
void setsekledPixel(uint8_t aktled, uint8_t aktledbck )
{ 
  uint8_t prevled;
  uint8_t aktsekbrightness = 0;
  uint16_t sekhue;
  color_s sek_color;
  const TickType_t xDelay = 900 / portTICK_PERIOD_MS;

  if ( settings.mySettings.sekmode == 1 )
  {
    sekhue = ( minute() * 60 + second()) * 0xFFFF / 3600;
    sek_color = num_to_color(sekledDriver.ColorHSV(sekhue));
  }
  if ( settings.mySettings.sekmode == 2 )
  {
    sek_color = defaultColors[settings.mySettings.color];
  }
  
  if ( settings.mySettings.sekmode == 3 )
  {
    sek_color = num_to_color(settings.mySettings.sekcolor);
  }
  
  if ( aktled == 0 ) 
  {
    prevled = NUMPIXELS_SEK-1;
  }
  else
  {
    prevled = aktled - 1;
...
//  delay(950);
  ...
  aktsekbrightness = brightness;
//  Serial.printf("aktled: %i prevled: %i baktsekbrightness: %i\n", aktled, prevled,aktsekbrightness);
  sekledDriver.setPixelRGB(prevled,0,0,0);
  sekledDriver.setPixelRGB(aktled,sek_color.red * aktsekbrightness/0x255 ,sek_color.green * aktsekbrightness/0x255,sek_color.blue * aktsekbrightness/0x255);
  delay(1);
  sekledDriver.show();
  delay(10);
  sekledDriver.show();
